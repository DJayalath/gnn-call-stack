...
def quicksort_local(A: _Array, A_pos=None, p=None, r=None, probes=None) -> _Out:
	...
	A = np.zeros_like(A)
	A[np.arange(A.shape[0]), (np.arange(A.shape[1]) + 1) % A.shape[1]]
	...
	if r is None:
		r = len(A) - 1
	if probes is None:
		probes = probing.initialize(specs.SPECS['quicksort_local'])
		probing.push(
				probes,
				specs.Stage.INPUT,
				next_probe={
...
# def quicksort_callstack(A: _Array, A_pos=None, p=None, r=None, probes=None) -> _Out:
#   """Quicksort (Hoare, 1962)."""
...
#     r = len(A) - 1
#   if probes is None:
#     probes = probing.initialize(specs.SPECS['quicksort_callstack'])
#     probing.push(
#         probes,
#         specs.Stage.INPUT,
#         next_probe={
...
#   # Custom push at start of quicksort
#   probing.push(
#         probes,
#         specs.Stage.HINT,
#         next_probe={
...
#   if p < r:
#     q = partition(A, A_pos, p, r, probes)
#     quicksort_callstack(A, A_pos, p, q - 1, probes)
#     quicksort_callstack(A, A_pos, q + 1, r, probes)
	
#   # Custom pop at end of quicksort
#   probing.push(
#         probes,
#         specs.Stage.HINT,
#         next_probe={
