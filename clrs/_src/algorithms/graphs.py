...
def dfs_recursive_callstack_hard(A: _Array) -> _Out:
  ...
  # Main DFS algorithm
  chex.assert_rank(A, 2)
  probes = probing.initialize(specs.SPECS['dfs_recursive_callstack_hard'])
...
# DFS baseline - without s_last and s_prev availability in hints!
def dfs_hard(A: _Array) -> _Out:
  """Depth-first search (Moore, 1959)."""
...
def dfs_callstack_localhints_no_stackhint(A: _Array) -> _Out:
  """Depth-first search (Moore, 1959)."""
  ...
  chex.assert_rank(A, 2)
  probes = probing.initialize(specs.SPECS['dfs_callstack_localhints_no_stackhint'])
...
def dfs_nohint_callstack(A: _Array) -> _Out:
  """Depth-first search (Moore, 1959)."""

  chex.assert_rank(A, 2)
  probes = probing.initialize(specs.SPECS['dfs_nohint_callstack'])
